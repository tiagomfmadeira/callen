...
def create_item(conn, item):
    ...
    sql = ''' INSERT INTO item(id, name, description, year, matrix, collection, date_inserted, date_modified, date_viewed, deleted, pic_path, archive_id)
              VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) '''
    cur = conn.cursor()
...
#################################################################################
	...
	conn = create_connection(r"CallenDB.db")
	...
	for child in root:
		...
		for tmp in child:
			if tmp.tag == "ID":
				...
				item_info[11] = tmp.text

		create_item(conn, tuple(item_info))
		i = i + 1
